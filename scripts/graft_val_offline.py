...
from mib_pipeline.graft import APPROVE_CONF_MAX, graft_row  # noqa: E402
...
def main() -> int:
    ...
    print(f"ms={len(ms)} st={len(st)} conf_max={conf_max}", flush=True)
    ...
    with out.open("w") as handle:
        for i in range(100001, 105001):
            ...
            if s is None:
                ...
                row = graft_row(hybrid, s, conf_max=conf_max)
                if row.get("adjudication") != before:
                    demoted += 1
            handle.write(json.dumps(row) + "\n")
    print(f"wrote {out} demoted={demoted}", flush=True)
    return 0
